      ...
      for (int i = 0; i < bLim; i++) {
        ...
        for (int d = 0; d < lm->UDIRS; d++) {
          ...
          if (lm->e[l][d] < 0) {
            int cf = c;
            if (lm->DIM > 2) {
              cf = (c + (lm->n.z - 1) * (l == 2) + lm->e[2][d] * (l != 2))
                  % lm->n.z;
              cf += lm->n.z * (cf < 0);
            }
            int bf = (b + (lm->n.y - 1) * (l == 1) + lm->e[1][d] * (l != 1))
                % lm->n.y;
            bf += lm->n.y * (bf < 0);
            int af = (a + (lm->n.x - 1) * (l == 0) + lm->e[0][d] * (l != 0))
                % lm->n.x;
            af += lm->n.x * (af < 0);
            ...
            int bf = (b + lm->e[1][d] * (l != 1)) % (lm->n.y)
                + (lm->n.y) * (((b + lm->e[1][d] * (l != 1)) % (lm->n.y)) < 0);
            int af = (a + lm->e[0][d] * (l != 0)) % (lm->n.x)
                + (lm->n.x) * (((a + lm->e[0][d] * (l != 0)) % (lm->n.x)) < 0);
            int cf = c;
            if (lm->DIM > 2) {
