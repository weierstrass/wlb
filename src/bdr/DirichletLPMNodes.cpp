...
template<class T>
void DirichletLPMNodes<T>::updateBorderNode(const bool unknowns[9], int x,
    int y, double val) {
...
//	cout<<"RHS: "<<cm->g_rhs(x, y)<<endl;
  ...
  for (int i = 0; i < 9; i++) {
    if (unknowns[i])
      f[0][y][x][i] = cm->fEq(i, psi0);
  }
}

/* update routine for corner nodes which need special treatment */
template<class T>
void DirichletLPMNodes<T>::updateCornerNode(const bool unknowns[9], int x,
    int y, double val) {
  double sp = 0, grhs = 0, psi0;
  for (int i = 0; i < 9; i++) {
    if (!unknowns[i])
