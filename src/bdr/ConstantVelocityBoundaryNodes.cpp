...
void ConstantVelocityBoundaryNodes::updateF(double ***f, double ***u, double **rho){
	int i,j;
	...
	//	cout<<"updating boundary node: ("<<i<<", "<<j<<")"<<", uy: "<<bNodes[k].valueY<<endl;
		...
		if(i == 0 && j == ny-1-1){//todo
			...
			double foo = 0.5*(rho[i][j] - (f[0][i][j] + f[1][i][j] + f[2][i][j] +
							  f[3][i][j] + f[4][i][j] + f[5][i][j] + f[7][i][j]) );
			f[6][i][j] = foo;
			f[8][i][j] = foo;
			//continue;
		}else if(i == 0 && j == 1){
			...
			rho[i][j] = rho[i+1][j+1];
			double foo = 0.5*(rho[i][j] - (f[0][i][j] + f[1][i][j] + f[2][i][j] +
					f[3][i][j] + f[4][i][j] + f[6][i][j] + f[8][i][j]) );
			f[5][i][j] = foo;
			f[7][i][j] = foo;
			//continue;
		}else if(i == nx-1 && j == 1){
			...
			u[X][0][j] = bNodes[k].valueX;
			u[Y][0][j] = bNodes[k].valueY;
			rho[0][j] = 1/(1 - u[X][0][j])*(f[0][0][j] + f[2][0][j] + f[4][0][j] +
			...
			u[X][i][j] = bNodes[k].valueX;
			u[Y][i][j] = bNodes[k].valueY;
			rho[i][j] = 1/(1 + u[X][i][j])*(f[0][i][j] + f[2][i][j] + f[4][i][j] +
							2*( f[1][i][j] + f[5][i][j] + f[8][i][j]));
			f[3][i][j] = f[1][i][j] - 2.0/3.0*rho[i][j]*u[X][i][j];
			f[7][i][j] = f[5][i][j] - 0.5*(f[4][i][j] - f[2][i][j]) - 1.0/6.0*rho[i][j]*u[X][i][j]
			              			                                - 0.5*rho[i][j]*u[Y][i][j];
			f[6][i][j] = f[8][i][j] + 0.5*(f[4][i][j] - f[2][i][j]) - 1.0/6.0*rho[i][j]*u[X][i][j]
			              			                                + 0.5*rho[i][j]*u[Y][i][j];
		}else if(j == ny-1){
			u[X][i][j] = bNodes[k].valueX;
			u[Y][i][j] = bNodes[k].valueY;
			rho[i][j] = 1/(1 - u[Y][i][j])*(f[0][i][j] + f[1][i][j] + f[3][i][j] +
										2*(f[4][i][j] + f[7][i][j] + f[8][i][j]));

			f[2][i][j] = f[4][i][j] + 2.0/3.0*rho[i][j]*u[Y][i][j];
			f[5][i][j] = f[7][i][j] - 0.5*(f[1][i][j] - f[3][i][j]) + 0.5*rho[i][j]*u[X][i][j]
			                                                        + 1.0/6.0*rho[i][j]*u[Y][i][j];
			f[6][i][j] = f[8][i][j] + 0.5*(f[1][i][j] - f[3][i][j]) - 0.5*rho[i][j]*u[X][i][j]
			                                                        + 1.0/6.0*rho[i][j]*u[Y][i][j];
		}else if(j == 0){
			u[X][i][j] = bNodes[k].valueX;
			u[Y][i][j] = bNodes[k].valueY;
			...
			f[7][i][j] = f[5][i][j] + 0.5*(f[1][i][j] - f[3][i][j]) - 0.5*rho[i][j]*u[X][i][j]
			                                                        - 1.0/6.0*rho[i][j]*u[Y][i][j];
			f[8][i][j] = f[6][i][j] - 0.5*(f[1][i][j] - f[3][i][j]) + 0.5*rho[i][j]*u[X][i][j]
			                                                        - 1.0/6.0*rho[i][j]*u[Y][i][j];

		}else{
			cerr<<"No implementation found for requested constant velocity boundary node!"<<endl;
		}
	}
}
...
void ConstantVelocityBoundaryNodes::addNode(int x, int y, double vx, double vy){
