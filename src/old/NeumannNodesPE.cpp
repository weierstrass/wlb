...
void NeumannNodesPE::updateF() {
  ...
  for (int i = 0; i < nodes.size(); i++) {
    ...
    v->v1 = getDirichletValue(v->x, v->y, lm->ex[(int) v->v3],
        lm->ey[(int) v->v3], neumanValue) / 1.8;
...
double NeumannNodesPE::fEq(int dir, int i, int j) {
  CollisionD2Q9LPMChaiRHS *com =
      dynamic_cast<CollisionD2Q9LPMChaiRHS*>(this->cm);
