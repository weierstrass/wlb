...
void CollisionD2Q9BGKShanChenForce::collide() {
  ...
  for (int j = 0; j < n.y; j++) {
    for (int i = 0; i < n.x; i++) {
      ...
      for (int d = 0; d < 9; d++) {
        f[0][j][i][d] += w
            * (fEq(d, rho, rhoU[X] / rho, rhoU[Y] / rho) - f[0][j][i][d]);
...
double *CollisionD2Q9BGKShanChenForce::getRhoUEq(double *f, double ffx,
    double ffy) {
...
double *CollisionD2Q9BGKShanChenForce::getRhoU(double *f, double ffx,
    double ffy) {
  double *u = new double[3];
