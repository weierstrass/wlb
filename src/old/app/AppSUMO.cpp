...
void updateForce(double **fx, double **fy, double **ux, double **uy,
    double **rho_eps, Lattice2D *lm, double eps_r, double u0, double l0,
    double V0, double C0, double bulkCond, double dPdx,
    CollisionD2Q9AD *cmNPpos, CollisionD2Q9AD *cmNPneg);

void addRect(int x, int y, int w, int h, double rho_surface,
    BounceBackNodes<CollisionD2Q9BGKNSF> *bbNS, NeumannNodesPESlip *bds,
    SlipNodes<CollisionD2Q9AD> *bbnNeg, SlipNodes<CollisionD2Q9AD> *bbnPos);
...
void updateForce(double **fx, double **fy, double **ux, double **uy,
  ...
  double jx = 0;
  double jy = 0;
  double prefactorJ = PHYS_E_CHARGE * C0 * rs * u0 / bulkCond * l0
      / (u0 * u0 * rho0);
