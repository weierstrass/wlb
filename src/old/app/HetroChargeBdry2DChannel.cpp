...
void updateForce(double **fx, double **fy, double **ux, double **uy,
    double **rho_eps, Lattice2D *lm, double eps_r, double u0, double l0,
    double V0, double C0, double bulkCond, double E, CollisionD2Q9AD *cmNPpos,
  ...
  for (int j = 0; j < lm->n.y; j++) {
    for (int i = 0; i < lm->n.x; i++) {
      ...
      fx[j][i] = prefactorJ * jx * rho_eps[j][i]
          + prefactorE * rho_eps[j][i] * E;
